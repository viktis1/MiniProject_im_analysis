...
test_array = np.stack([mask, mask,mask], axis=3).reshape(np.shape(best_direction)[0],np.shape(best_direction)[1],np.shape(best_direction)[2],-1).astype(bool)
best_direction = best_direction[test_array].reshape(-1,3)

max_rgb = np.max(np.abs(best_direction))
...
# print(np.shape(colorspace))
# print(np.shape(test_array))
# print(colorspace[mask,:])

# print(np.shape(colorspace[test_array]))
